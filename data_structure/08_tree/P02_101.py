...
class Solution:
    def get_values_by_level(self, values, level):
        left = 2 ** (level - 1) - 1
        count = 2 ** (level - 1)
        return values[left: left + count]
    ...
    def isSymmetric(self, root: Optional[TreeNode]) -> bool:
        ...
        for level in range(1, tree.height() + 1):
            if not self.is_palindrome(self.get_values_by_level(values, level)):
                return False
        return True


tree = BinaryTree([1, 2, 2, 3, 4, 4, 3])
tree.display()
